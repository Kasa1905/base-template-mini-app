...
import Link from 'neexport default function DemoPage() {
  const { theme } = useTheme();
  const [selectedCredential, setSelectedCredential] = useState(null);
  const [showQRModal, setShowQRModal] = useState(false);
  const [currentDemo, setCurrentDemo] = useState('browse'); // 'browse', 'create', 'verify'link';
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-primary transition-all duration-300">
        {/* Header */}
        <nav className="bg-secondary shadow border-b border-themed">
